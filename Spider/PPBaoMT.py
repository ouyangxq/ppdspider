...
class PPBaoMT(object):
    ...
    def init(self):
        ...
        self.loanids_in_memory = self.loandao.get_last_2_days_loanids()
        ...
        for page in range(page_start, page_end):
            ...
            for loanid in loanid_to_mobile.keys(): 
                ...
                if (loanid in self.loanids_in_memory):
                    ...
                    html = self.spider.open_loan_detail_page(loanurl, pageurl)
                    last_url = loanurl
                    if (html is None):
                        self.error_count += 1
                        st = random.randint(2,7)
                ...
                for ppdid in self.ppdloginids:
                    ...
                    if ((bidmoney is not None) and (self.ppdid_to_leftmoney.has_key(ppdid)) and (self.ppdid_to_leftmoney[ppdid] < bidmoney)):
                        ...
                        if self.NOBID == True:
                            ...
                            # Actually bid for it
                            (actual_bid_money, mymoney_left) = self.autobid.bid(self.ppdid_to_spider[ppdid].opener, ppdloan.loanid, ppdloan.maturity, bidmoney, reason, bid_strategy.strategy_name)
    ...
    def create_page_worker_threads(self, risk, pages, loan_queue, iter=10):
        num_of_threads = int(pages / iter) if (pages % iter == 0) else int(pages / iter + 1)
        plist = [1]
    ...
    def run(self):
        ...
        check_and_bid_thread = Thread(target=self.check_and_bid, args=(loan_queue,))
        ...
        while (1):
            ...
            for risk in [self.spider.riskmiddle]: # 20160307: remove: spider.risksafe as I already bid more than 3000 for 12/12 Peibiao 
                try: 
                    ...
                    page_reader_threads = []
                    page_reader_threads = self.create_page_worker_threads(risk, pages, loan_queue, 10)
